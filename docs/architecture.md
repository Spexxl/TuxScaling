...
The current workspace captures supported swapchains, computes estimated optical flow and guidance on RADV, tracks temporal history, optionally reduces only the internal guidance work, resolves all backend-visible guidance to the game extent, runs either the vendor-neutral reference reconstruction or the experimental FSR 3.1.4 backend through the generic contract, and injects an egui diagnostic panel with X11/XWayland input. Native or fixed output resolution creates a layer-owned X11/XWayland presenter window and physical swapchain while preserving the game's logical window and swapchain images; `swapchain` and native Wayland remain direct. The legacy reversible borderless lease is retained only for fail-open compatibility paths and is not used by the successful presenter path. `vkcube` and the WSI harness are the validation targets for grouped presents, resize, presenter replacement, restoration, and synchronization. DLSS/XeSS integrations remain separate milestones. See [Vulkan Layer Runtime](vulkan-layer-runtime.md) for its maintenance contract.
